"""
Restaurant Agent Executor

Handles A2A protocol execution and converts LLM output to A2UI parts.
"""
...
class RestaurantAgentExecutor(AgentExecutor):
    """Restaurant AgentExecutor for A2A protocol."""
    ...
    async def execute(
        ...
        if use_ui:
            agent = self.ui_agent
            logger.info("--- A2UI extension is active. Using UI agent. ---")
        else:
            agent = self.text_agent
            logger.info("--- A2UI extension is not active. Using text agent. ---")

        if context.message and context.message.parts:
            logger.info(
                f"--- Processing {len(context.message.parts)} message parts ---"
            )
        ...
        if ui_event_part:
            ...
            if action == "book_restaurant":
                ...
                query = f"Show me details for restaurant with id: {restaurant_id}"
